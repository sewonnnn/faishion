...
import axios from "axios";
import {Button} from "react-bootstrap";
import React, {useMemo} from "react"; // useMemo를 import합니다.
...
const CartPage = () => {
    ...
    // ⭐ 3. useMemo를 사용하여 선택된 상품들의 가격만 계산
    ...
    }, [cartList, selectedItems]); // cartList 또는 selectedItems가 변경될 때만 재계산

    // ... (goSelectedItemsOrder, onAIForm, getOrderSummary, etc.)
    const goSelectedItemsOrder = async () => {
        ...
        try {
            ...
            // ⭐ axios 호출은 이미 선택된 상품만 가져오므로 변경 불필요
            const res = await axios.get("http://localhost:8080/order/new", {
                params: { ids: cartIds },
            });
            res.data.map((item) => {console.log(item)});
